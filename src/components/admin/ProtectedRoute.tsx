...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({
      ...
      <div className="min-h-screen bg-[#071322] flex flex-col items-center justify-center text-slate-100 font-sans p-6">
        ...
        <div className="text-center max-w-sm space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-900/30 border border-blue-700/40 text-blue-400 text-xs font-semibold">
            ...
            <span>Supabase Session Guard</span>
          </div>
  ...
  // 2. Unauthenticated: No user or active Supabase session -> Render secure Login Portal
  if (!user && !session) {
    if (fallback) {
      return <>{fallback}</>;
    }
  ...
  // 3. Optional Role Enforcement: User authenticated but lacks required role
    ...
    return (
      <div className="min-h-screen bg-[#071322] flex flex-col items-center justify-center p-6 text-center text-slate-100 font-sans">
        ...
        <button
          ...
          className="bg-amber-400 hover:bg-amber-300 text-slate-950 text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-lg"
        >
  ...
  // 4. Authenticated & Verified: Render protected dashboard routes
  return <>{children}</>;
};
