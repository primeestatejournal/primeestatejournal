...
export const PropertyCard: React.FC<PropertyCardProps> = ({
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-200/90 overflow-hidden shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col h-full hover:border-slate-300">
      ...
      <div 
        className="relative h-48 sm:h-52 overflow-hidden bg-gradient-to-br from-slate-900 via-[#0B1F3A] to-slate-950 p-4 flex flex-col justify-between cursor-pointer group-hover:from-slate-950 group-hover:to-blue-950 transition-all duration-300 border-b border-slate-800"
        onClick={() => onSelectProperty(property)}
      >
        {/* Subtle Geometric Blueprint Grid Lines */}
        ...
        {property.images && property.images.length > 0 ? (
          ...
          <div className="relative z-10 flex flex-col justify-between h-full">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                className={`p-2 rounded-full backdrop-blur-md border transition-all cursor-pointer ${
                  isSaved
                    ? 'bg-[#D4A72C] text-slate-950 border-amber-300 shadow-sm'
            ...
            {/* Middle Distinctive Identity Emblem */}
            <div className="my-auto py-2">
              <div className="flex items-center gap-2 text-slate-300">
                <div className="w-8 h-8 rounded-xl bg-blue-600/30 border border-blue-400/30 flex items-center justify-center text-[#155EEF]">
                  <Building2 className="w-4 h-4 text-blue-300" />
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[#D4A72C]">PRIME VERIFIED DOSSIER</p>
                  <p className="text-xs font-semibold text-white truncate max-w-[200px]">{property.titleNumber}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 pt-2 border-t border-white/10">
              <span className="bg-white/10 text-slate-200 text-[10px] font-medium px-2 py-0.5 rounded border border-white/10">
                {property.sizeDisplay}
              </span>

              <span className="bg-[#155EEF]/80 text-white text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1 border border-blue-400/40">
                <Scale className="w-3 h-3 text-[#D4A72C]" />
                <span>Risk: {property.dossier.legalRiskScore}/100</span>
              </span>
            </div>
          </div>
        )}

        {/* If image exists, render overlay badges */}
        {property.images && property.images.length > 0 && (
          <>
